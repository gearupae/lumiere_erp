...
class App_items_table extends App_items_table_template
{
    ...
    public function items()
    {
        ...
        foreach ($this->items as $item) {
            ...
            if($this->type == 'proposal'){
                $profit = (floatVal($item['rate']) - floatVal($item['cost']));
                $sub_profit = (floatVal($item['rate']) - floatVal($item['cost'])) * floatVal($item['qty']);
                if($item['cost'] != 0){
                    $profit_percent = intval(($profit/$item['cost']) * 100);
                }else{
                    $profit_percent = 100;
                }
                $total_profit_percentage = $total_profit_percentage + $profit_percent;
        ...
        <td colspan = 5></td>
                            ...
                            <td colspan = 3>
                                <span class="bold tw-text-neutral-700">Total Profit </span>
                            </td>
                            <td class="subtotal" colspan = 2>
                                '.app_format_money($total_profit,"AED") .'
                            </td>
                        </tr>
                        <tr>
        <td colspan = 5></td>
                            ...
                            <td colspan = 3>
                                <span class="bold tw-text-neutral-700">Total Profit Percentage </span>
                            </td>
                            <td class="subtotal" colspan = 2>
    ...
    public function html_headings()
    {
        ...
        if($this->type == 'proposal'){
        $html .= '<th align="right">Cost</th>';
        $html .= '<th align="right">Profit</th>';
        $html .= '<th align="right">Profit %</th>';
        }
