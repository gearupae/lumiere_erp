...
abstract class App_pdf extends TCPDF
{
    ...
    public function Footer()
    {
        ...
        if (get_option('show_page_number_on_pdf') == 1) {
         // Set fill color for a rectangle in the footer
         $this->SetFillColor(20, 164, 233);
         // Reduce the width of the rectangle from the left and right
        // Use the full page width for the rectangle
        $this->Rect(0, $this->getPageHeight() - 10, $this->getPageWidth() - 0, 10, 'F');
        
         // Footer text
         $this->SetY($this->getPageHeight() - 12);
         $this->SetTextColor(255, 255, 255); // White
         // Change font style to normal
         $this->SetFont($this->get_font_name(), '', 8);
         // Use Cell instead of MultiCell to prevent automatic line breaks
         $this->Cell(200, 15, "+971 2 562 2652 | info@lumiere-emc.com | www.lumiereemc.com | Headoffice: Khaldiya Abu dhabi UAE | Warehouse: Mussafah Abu dhabi UAE | Page:". $this->getAliasNumPage() . '/' . $this->getAliasNbPages(), 0, false, 'C', 0, '', 0, false, 'T', 'M');
        }
    }
