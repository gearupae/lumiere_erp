...
//     $info_right_column .= '<br /><span style="color:rgb(' . invoice_status_color_pdf($status) . ');text-transform:uppercase;">' . format_estimate_status($status, '', false) . '</span>';
// }
...
$estimate_info .= '<br />' . _l('invoice_data_date') . ': ' . _d($estimate->date) . '<br />';

if (!empty($estimate->expirydate)) {
    $estimate_info .= _l('invoice_data_duedate') . ': ' . _d($estimate->expirydate) . '<br />';
}
...
<table  width="100%" bgcolor="#fff" cellspacing="0" cellpadding="8">
   <thead>
      <tr height="30" bgcolor="red" style="color:black;">
...
// if($estimate->show_rate == 1){
    ...
    // $items_html .= '<th width="12%" align="right">Tax</th>';
// }else{
...
$items_html .=' <th width="14%" align="right">Amount</th>
   ...
   foreach ($groupedItems  as $key => $items) {
    ...
    //     $key = "Others";
    // }
    // $items_html .= ' <tr height="30" cellpadding="6" bgcolor="red" style="color:#ffffff;">
    ...
    $items_html .= '<tr style="font-size:14px;">
            ...
            <td class="description" align="left;" width="53%"><span style="font-size:13px;"><strong>'.$item['description'].'</strong></span><br><span style="color:#424242;">'.$item['long_description'].'</span></td>
    ...
    // if($estimate->show_rate == 1){
        ...
        // $items_html .='<td class="tax" align="right" width="8%">'.app_format_number($item['taxrate'],1) . '%.</td>';
