...
class Estimates_model extends App_Model
{
    ...
    public function update($data, $id)
    {
        ...
        foreach ($items as $key => $item) {
            ...
            if (update_sales_item_post($item['itemid'], $item, 'group_name')) {
                $this->log_estimate_activity($id, 'invoice_estimate_activity_updated_item_group_name', false, serialize([
                    $original_item->group_name,
                    $item['group_name'],
                ]));
                $affectedRows++;
            }
